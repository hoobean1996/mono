...
public:
  ...
  void decrease_height(node *ptr, node *node_in_underflow, int pos) {
    if (pos != ptr->count) {
      if (ptr->children[pos]->count <
          floor(BTREE_ORDER / 2.0)) { // underflow desde la izquierda
    ...
    sibling->insert_in_node(
        last, ptr->data[pos - 1]); // insert en la ult pos data padre
    sibling->children[last + 1] =
        node_in_underflow->children[0]; // el ptr al child en underflow
  ...
  bool merge_with_parent(node *ptr, node *node_in_underflow, int pos) {
    ...
    if (pos != 0) {
      ...
      if ((ptr->children[pos - 1]->count - 1) >=
          floor(BTREE_ORDER / 2.0)) { // puede prestar 1 dato?
        node *sibling = ptr->children[pos - 1];
        ...
        // pasamos el ult hijo del hermano para que ahora sea el 1er hijo del
        // node_in_underflow
      ...
      if ((ptr->children[pos + 1]->count - 1) >= floor(BTREE_ORDER / 2.0)) {
        ...
        // pasamos el 1er hijo del hermano para que ahora sea el 2do hijo del
        // node_in_underflow
  ...
  bool steal_sibling(node *node_in_underflow, node *ptr, int pos) {
    if (ptr->children[0]->children[0] ==
        NULL) { // solo puedes robar en nodos hojas
      if (pos != ptr->count) {
        if (ptr->children[pos + 1]->count - 1 >= floor(BTREE_ORDER / 2.0)) {
      ...
      if (pos > 0) {
        if (ptr->children[pos - 1]->count - 1 >= floor(BTREE_ORDER / 2.0)) {
          ...
          T jesus =
              sibling->data[sibling->count - 1]; // el valor mas a la derecha
          sibling->delete_in_node(sibling->count - 1);
          node_in_underflow->insert_in_node(0, jesus);
  ...
  T succesor(node *ptr) {
    ...
    if (ptr->count == 1) {
      if (ptr->right == NULL)
        return -1; // en el caso de encontrar el succesor del mayor valor
